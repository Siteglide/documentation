...
Some parameters that we consider non-sensitive normally, like those which might be used to filter data could in some use cases be something you do not wish users to be able to modify. In this case, you can use our [Enforcing Filters](live-updates-example-enforcing-filters.md) Guide to solve this problem.

See the `collection` and `creator_id` [URL parameters](live-updates-reference.md#get-url-parameters) for security considerations regarding JSON data and filtering items by their owner respectively.
